...
```bash
# 创建 volume（只需要执行一次）
fly volumes create mrbean9_bot_data --size 1 --region sin
```
...
1. **数据持久化**: 机器人通过 `DATA_DIR`（默认 `/data`）读写数据。`fly.toml` 中已将 volume `mrbean9_bot_data` 挂载到 `/data`，因此 `user_stats.json` 和 `admins.json` 会持久化保存，容器重启不会丢失。
